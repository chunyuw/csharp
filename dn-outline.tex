...
\item<5-| handout:1> C\# 和 ~CLI\only<5| handout:0>{, \textit{Common Language Infrastructure}}
  ...
  \only<5| handout:0>{\small 都已成为~ECMA 标准化的规范。Microsoft 为 ~XML Web 服务家族创建的一种编程
    语言 ~C\#，和~Microsoft .NET 框架的一个子集，叫做公共语言基础结构。这将使其他供应商能够在各种平台
...
\column{.58\textwidth}
\begin{itemize}[<+->]
\item .NET Framework 1.0 (2002-2-13)

\only<.>{ \CJKindent 初次发布，支持的系统包括 Win 98, NT 4.0, 2000, XP。}

\item .NET Framework 1.1 (2003-4-3)
...
\item .NET Framework 2.0 (2006-1-22)
...
\item .NET Framework 3.0 (2006-11-21)
...
\item .NET Framework 3.5 (2007-11-19)
...
\begin{exampleblock}{部分 ~CLS 特性和规则}<2-| handout:1>
  ...
  \begin{tabular}{l|l}
    ...
    {\hei特\ \ 性} & \multicolumn{1}{c}{\hei 规\ \ 则}          \\\hline
    可见性         & 定义类型的程序集之外有效                   \\
...
\item 程序集都有一个{\color<handout:0>{red!65!black}清单} (\textit{manifest}{}) \pause
\item 清单描述程序集的内容信息
  \begin{enumerate}
  \item 程序集的名字
...
\item 程序集定义了类型的有效范围， ~CLR 通过程序集查找类\pause
\item 程序集不需要注册表项， ~CLR 通过一种算法查找程序集\pause
...
\item 只有在需要的时候加载，程序集可以在本地或网络中\pause

\item CLR 通过特定的规则寻找需要的程序集\pause

  \begin{enumerate}
    \setlength{\itemsep}{4pt plus 1pt}
  \item CLR 检查是否已经被加载，如果已经加载，结束查找\pause
  \item 在全局程序集缓存(GAC)中查找\pause
  \item 在程序代码中指定的位置查找\pause
  \item 如果代码没有指定，在应用程序目录查找\pause
  \item 根据 ~.NET 的配置，在其他目录或 ~URL 查找\pause
  \item 如果都未找到，报告错误，结束查找\pause
  \end{enumerate}
...
\begin{itemize}
\item<1-| handout:1> 类库是通过{\color<handout:0>{red!60!black}命名空间} (\textit{namespace})构成的树形结构
\item<2-| handout:1> 命名空间 ~\texttt{System} 是树形结构的根
\item<3-| handout:1> 命名空间下可以有{\color<handout:0>{red!60!black}子命名空间}或{\color<handout:0>{red!60!black}类}
