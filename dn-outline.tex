...
\end{block}
\begin{itemize}
\item 加载 ~CLR 的程序称为{\color{red!65!black}运行库宿主} (\textit{runtime host})
\item 应用程序域，由运行库宿主创建
\item 宿主依靠{\color{red!65!black}验证} (\textit{verification})来隔离程序集
...
\frametitle{命名空间的结构}
  ...
  \begin{itemize}
  \item 类库是通过{\color{red!60!black}命名空间} (\textit{namespace})构成的树形结构
  \item 命名空间 ~System 是树形结构的根
  \item 命名空间下可以有{\color{red!60!black}子命名空间}或{\color{red!60!black}类}
...
\CJKindent \textbf{System} 是整个类库的根，除了容纳大量的子空间之外，还有很多基本的对象和类型
...
\item CTS 定义的基本数据类型
  ...
  \end{itemize}
\item \textbf{System.Console} 类，提供控制台（字符界面）的输入输出功能，如 ~\texttt{WriteLine()},
  \texttt{ReadLine()}等
\item \textbf{System.Math} 类，各种数学函数运算，如 ~\texttt{Sqrt()}, \texttt{Sin()}, \texttt{Ceiling()}, \texttt{Log()}等
\item \textbf{System.Enviroment} 类，用于提供当前运行环境信息，如操作系统、当前目录、内存使用量等
\item \textbf{System.Random} 类，用于产生随机数
...
\frametitle{常用的类库简介}
  \begin{itemize}
  \CJKindent
  \item \textbf{System.IO} --- 文件或目录的访问    \smallskip

     \small 基本的输入输出，文件、目录以及内存数据流的访问。
    \smallskip

  \item \textbf{System.Net} --- 网络数据的访问    \smallskip

     \small 使用各种网络协议 ~HTTP, FTP, Mail, 基本的 ~Socket 等进行网络数据通信，简化了
    网络应用程序的开发。
    \smallskip

  \item \textbf{System.Text} --- 文本处理    \smallskip

     \small 提供不同编码文本之间的转换功能，文本处理功能，正则表达式搜索及匹配文本等等。
    \smallskip

  \item \textbf{System.Data} --- ADO.NET 的基础    \smallskip
...
\begin{block}{}
  \texttt{csc /t:exe /out:Hello.exe MyHello.cs} 
\end{block}
