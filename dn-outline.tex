...
\begin{frame} \frametitle{公共语言基础结构(CLI)}
  \framesubtitle{实现 ~.NET 平台的基础}
  \begin{block}{\textit{Common Language Infrastructure}}
...
\begin{frame} \frametitle{公共语言运行环境}
  \begin{block}{\textit{Common Language Runtime}}
    \CJKindent 全部 ~.NET Framework 的基础，为 ~.NET 程序提供了一个虚拟机环境，
    以支持进程的创建和执行，为进程提供必要的服务。
  \end{block}
  % \input{pgf-dn/clr-compile-exec}
  \begin{itemize}
  \item<2-| handout:1> CLR 要管理程序运行的整个周期：查找代码、编译、加载相关的类、管理其
    执行、自动内存管理
  \item<3-| handout:1> 提供多种语言都支持的公共类型系统，定义如何声明和管理这样的类型
