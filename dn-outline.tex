...
\begin{frame} \frametitle{公共语言结构(CLI)}
  \begin{block}{Common Language Infrastructure, CLI}
    \CJKindent CLI 标准定义了一个与平台无关的虚拟代码执行环境。包括公共中间语
    言(CIL)和公共类型系统(CTS)，相应的语法、语义和部分 .NET 框架类库。
  \end{block}
  \begin{itemize}
  \item CLI 没有指定操作系统，可以在任何的系统或设备上实现
  \item 任何人或公司都可以根据CLI标准实现相关技术
    \begin{itemize}
    \item Microsoft .NET Framework
    \end{itemize}
  \item 目前已有的开源 .NET 平台
...
\begin{lstlisting}<1>[language=XML, showstringspaces=false]
...
\begin{lstlisting}<2>[language=XML, showstringspaces=false]
HTTP/1.1 200 OK
Content-Type: text/xml; charset=utf-8
Content-Length: length
...
\end{frame}
\begin{frame}\frametitle{Microsoft .NET Framework}
  \framesubtitle{提供了丰富的类库}

  Windows Forms, ASP.NET, Enterprise Services, ADO.NET ...

  \begin{itemize}
  \item \large 充分利用代码复用，简化了应用程序的开发
  \item \large 类库提供开发所需的各种基本功能，可以支持各种类型的应用程序
  \item \large 类库屏蔽了底层细节，统一了编程模式，易于使用
  \item \large 容易维护、部署等
  \end{itemize}
\end{frame}
\begin{frame}\frametitle{Microsoft .NET Framework}
  \framesubtitle{完整的安全解决方案}
  \begin{itemize}
  \item \large 基于证据的安全规则，确保组件的来源可靠
  \item \large 细致的权限设置，可以为组件设置不同权限
    \begin{itemize}
    \item \large 访问注册表
    \item \large 文件读取
    \item \large 访问网络
...
\begin{frame} \frametitle{公共语言规范(CLS)}
  \begin{block}{Common Language Specification, CLS}
    语言互操作的最终规范，定义了遵循~CLR~的编译器必须包含的最小特性集，保证语言
    之间的通信。
  \end{block}
