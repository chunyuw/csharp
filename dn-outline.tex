...
托管代码是由面向 ~CLR 的编译器生成，支持公共语言运行时服务的代码，为了支持这些服务，代码必须向运行时提供最小级
...
\frametitle{程序集的功能}
  \begin{itemize}
  \item 程序集定义了类型的有效范围， ~CLR 通过程序集查找类的定义
  \item 程序集不需要注册表项， ~CLR 通过一种算法查找程序集的位置 
...
\begin{block}{共享程序集}
  \CJKindent \small 共享程序集是可被多个应用程序引用的程序集，通常被明确安装在全局程序集缓存 (\textit{Global
    Assembly Cache})中。要共享一个程序集，该程序集必须明确为这个目的而创建，这可以通过给其指定加密的增强型名称
  来实现。
\end{block}
