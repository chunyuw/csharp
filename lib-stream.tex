...
\item System.Net
\begin{itemize}
\item WebClient, WebResquest/WebResponse, Dns, \ldots
\end{itemize}
\item System.Net.Socket
\begin{itemize}
\item Socket, TcpListener, TcpClient, UdpClient
\end{itemize}
\end{itemize}
...
\frametitle{Web访问}
\begin{itemize}
\item WebRequest/WebResponse 可以用来处理 HTTP 和 FTP 客户端访问
...
{ buf = new byte[5];
  ...
  using (n = client.GetStream()){
    n.BeginRead(buf, 0, 5, call, null);   }
}
static void callfunc(IAsyncResult r) {
  int len = n.EndRead(r);
  if (len > 0) {
