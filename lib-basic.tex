...
在命名空间~System.Collections.Generic 中
...
即使用静态类~System.Linq.Enumerable 的三个扩展方法
\begin{lstlisting}
...
\begin{frame}[fragile]
\frametitle{Enumerable~的扩展方法}

\CJKindent 其中用到的是静态类~System.Linq.Enumerable 中对接口 IEnumerable$<$T$>$ 的几个扩展方法
...
\frametitle{强制立即执行}
\begin{itemize}
\item 对查询表达式取值，触发~LINQ 的执行
\item 也可以通过~ToArray$<$T$>$() 或~ToList$<$T$>$() 等方法
\begin{lstlisting}[escapeinside='']
