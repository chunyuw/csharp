...
\begin{frame}[fragile]
\frametitle{自动特性~(\textit{Automatic Properties})}
  \CJKindent 在创建特性的同时，自动创建私有字段。“私有字段$+$公有特性”可以为对
  象提供良好的封装性。
...
\begin{frame}[fragile]
\frametitle{匿名类型}
\begin{block}{\textit{Anonymous types}}
