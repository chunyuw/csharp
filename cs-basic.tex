...
\CJKindent 以创建传统的 Windows 客户端应用程序、XML Web services、分布式组件、客户端 - 服务器应用程序、数据库应用程序以及很多其他类型的程序
...
\frametitle{枚举类型}
\begin{itemize}
\item 定义常量可以用 \texttt{const} 关键字\\
  \lstinline|const int x = 100;|
\item 也可以使用枚举类型
\begin{lstlisting}
enum Color
{Red, Blue=2, Green}
...
int[] x = new int[] {1,3,5,7,9}; // <初始化>

int[,] y = new int[2,5]; // $2\times5$ <的数组>
\end{lstlisting}
\end{itemize}

  \begin{figure}[h] \label<1| handout:1>{fig:cs-array-mul}
    \centering \input{pgf-dn/cs-array-mul}
  \end{figure}
...
\end{itemize}
  ...
  \begin{figure}[h] \label<1| handout:1>{fig:cs-array-rec}
    \centering \input{pgf-dn/cs-array-rec}
  \end{figure}
\end{frame}

\begin{frame}[fragile]
\frametitle{数组的使用}
数组类型在类库中称为 ~\texttt{System.Array}，提供的方法
