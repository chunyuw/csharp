\begin{tikzpicture}[rounded corners,font=\small]
  \draw[fill=yellow!10,opacity=.5] (  0,  0) rectangle +(11, 5.5); % background
  \filldraw[black!20!white] (.8,.2) rectangle +(9.4, .6); 
  \node at (5.5, .5) {操作系统或移动设备};

  \draw[dashed] (.6,1) rectangle +(9.8, 3.2);
  \filldraw[red!20] (.8,1.2) rectangle +(9.4, .6); 
  \node at (5.5, 1.5) {公共语言运行环境CLR};
  \filldraw[blue!20] (.8,  2) rectangle +(9.4,  2); 
  \node at (5.5, 2.3) {.NET 框架类库FCL};
