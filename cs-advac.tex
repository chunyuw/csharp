...
\end{lstlisting}
\begin{itemize}
\item 接口中的方法也可以使用 ~new 关键字隐藏
\end{itemize}
\end{frame}

\begin{frame}[fragile]
\frametitle{显示实现接口}
 接口默认是~ public，如果需要 ~private 要使用显示实现
\begin{lstlisting}
using System;
interface IEven {
  bool isOdd(int x);
...
\begin{frame}[fragile]
\frametitle{自动类型转换}
C\# 2.0 中，可以省略 ~new 表达式，直接使用方法名创建委托实例，添加委托
\begin{lstlisting}
public static void Main() {
...
\item 一般来说多播委托的返回值应该是 ~void，不用参数列表返回参数
\item 添加和去掉方法的格式比较自由
\end{itemize}
\begin{lstlisting}
...
\item 定义为事件后，即使是 ~\texttt{public}，其他组件也只能订阅和删除
